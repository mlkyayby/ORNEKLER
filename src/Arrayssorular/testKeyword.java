...
*/
    ...
    private static void testSonucu(char[][] answers, char[] keys) {
        ...
        for (int i = 0; i <answers.length ; i++) {//her bitr ogrenciye bakar
            int sayac=0;// her bir ogrenci icin icin dogru cevaplari sayacagi icin  sayac buraya konur
            for (int j = 0; j <answers[i].length; j++) {//her ogrencinin cevaplarina bakar
               if (answers[i][j]==keys[j]){//cevap anahtari ile ogrenci cevaplarini karsilastiriryor
                   //cevaplar j old icin keys(j) olmali
                   sayac++;
